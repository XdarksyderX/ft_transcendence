...
//import { loadLogin } from '../login/login.js';
  ...
  function toggle2FASwitch(event) {
    ...
    if (event) { //only when the toggle comes from an event there's a change
      const enable = isChecked !== currentStatus ? true : false;
      toggleChanges(enable, changes.enable2FA, isChecked);
    }
  }

  function toggleChanges(enable, field, content) {
  ...
  function initUsernameChangeEvents() {
      ...
      usernameForm.addEventListener("submit", (event) => {
          ...
          toggleChanges(enableBtn, changes.username, newUsername);
      });
