...
class SendRequestView(APIView):
    ...
    def post(self, request):
        target_user = get_object_or_404(User, username=request.data.get("friend_username"))
        ...
        if target_user.blocked.filter(id=request.user.id).exists():
            return Response({"status": "error", "message": "You cannot send a friend request to this user."}, status=status.HTTP_400_BAD_REQUEST)
        
        if request.user.friends.filter(id=target_user.id).exists():
            return Response({"status": "error", "message": "This user is already your friend."}, status=status.HTTP_400_BAD_REQUEST)
